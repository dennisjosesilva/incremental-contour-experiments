...
def main(OUTPUT_FILE_NAME= "runtime-icdar-val/runtime_0.csv",
         ...
         EXECUTATION_ORDER = "rhb", SEED=42, NSAMPLES=50):
  ...
  def extract_programs_calls():
    ...
    lfiles = np.array(sorted(lfiles))
    
    rgen = np.random.RandomState(SEED)    
    idx = rgen.permutation(len(lfiles))
    lfiles = lfiles[idx[:NSAMPLES]]
...
parser.add_argument("-n", "--nsamples", type=int,
                    help="number of images to be analysed",
                    default=50)
